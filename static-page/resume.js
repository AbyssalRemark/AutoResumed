...
function loadEntry(entry,className) {
    ...
    let prototypes = document.getElementById("element-prototypes")
    let form = resumeForm.getElementsByClassName(className);
    let prototype = prototypes.getElementsByClassName(className);
    form[0] = prototype[0];
...
function collectEntries(className) {
    ...
    for (a = 0; a < elements.length; a++) {
        ...
        for (b = 0; b < fields.length; b++) {
            ...
            else if (key=="highlights" || key=="keywords" || key=="courses"){
...
function clearResume(){
   let resume = document.getElementById("resume-form")
   resume.innerHTML=""
}
